...
export const SAB_CHAT_TOOLS: Anthropic.Tool[] = [
  ...
  {
    name: 'create_payslip',
    description: 'Create an ATO-compliant payslip for an employee using NAT 1004 PAYG calculations. Use when the user asks to pay, process payroll, or make a payslip. Always call this first — never send without confirming. Returns the full breakdown with tax, Medicare, super.',
    input_schema: {
      type: 'object' as const,
      properties: {
        employee_id:      { type: 'string', description: 'UUID of the employee from the employees table' },
        gross_pay:        { type: 'number', description: 'Gross pay for this period in AUD' },
        pay_period_start: { type: 'string', description: 'Start date of pay period in YYYY-MM-DD format' },
        pay_period_end:   { type: 'string', description: 'End date of pay period in YYYY-MM-DD format' },
      },
      required: ['employee_id', 'gross_pay', 'pay_period_start', 'pay_period_end'],
    },
  },
  {
    name: 'send_payslip',
