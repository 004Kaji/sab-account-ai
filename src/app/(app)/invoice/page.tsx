...
function calcTotals(items: LineItem[]) {
  ...
  return { subtotal_ex_gst: Math.round(subtotal_ex_gst * 100) / 100, total_gst: Math.round(total_gst * 100) / 100, total_inc_gst }
}
...
function InvoicePreview({ form, biz, totals }: {
  ...
  return (
    <div style={{
        ...
        <tbody>
          {form.line_items.map((item, i) => (
            <tr key={item.id} style={{ background: i % 2 === 1 ? '#FAF8F5' : 'transparent' }}>
              <td style={{ padding: '0.375rem 0.5rem', maxWidth: '160px', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                {item.description || '—'}
              </td>
      ...
      {/* Notes */}
      {form.notes && (
        <p style={{ color: '#646060', fontSize: '0.75rem', whiteSpace: 'pre-line' }}>{form.notes}</p>
      )}

      <p style={{ color: '#C0BAB5', fontSize: '0.625rem', textAlign: 'center', marginTop: '1.5rem' }}>
...
export default function InvoicePage() {
  ...
  // Load business profile + next invoice number on mount
  useEffect(() => {
    async function load() {
  ...
  const setField = useCallback(<K extends keyof InvoiceForm>(key: K, value: InvoiceForm[K]) => {
    setForm(prev => {
      ...
      // Auto-update due_date when payment_terms or issue_date changes
    ...
    return (
      <div style={{ maxWidth: '520px', margin: '4rem auto', padding: '0 1.5rem', textAlign: 'center' }}>
        ...
        <p style={{ color: 'var(--text2)', fontSize: '0.9375rem', marginBottom: '2rem' }}>
          {savedInvoice.number} · {formatCurrency(totals.total_inc_gst)} has been saved.
        </p>
    ...
    <div style={{ maxWidth: '1280px', margin: '0 auto', padding: '2rem 1.5rem' }}>
      ...
      <div style={{ marginBottom: '1.75rem' }}>
        ...
        <p style={{ color: 'var(--text3)', fontSize: '0.875rem' }}>
          {form.invoice_number} · {biz?.business_name || profile.business_name || 'Your Business'}
        </p>
      </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
          ...
          {mode === 'ai' && (
            <div style={{ background: '#ffffff', borderRadius: 'var(--r)', border: '1px solid rgba(200,75,47,0.25)', padding: '1.25rem' }}>
              <label className="sab-label" style={{ marginBottom: '0.5rem', display: 'block' }}>
              ...
              <p style={{ fontSize: '0.75rem', color: 'var(--text3)', marginTop: '0.5rem' }}>
                Claude AI will create line items from your description. Review before saving.
              </p>
            </div>
          )}
          ...
          <div style={{ background: '#ffffff', borderRadius: 'var(--r)', border: '1px solid var(--border)', padding: '1.25rem' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
              <div>
                <label className="sab-label">Client Name <span style={{ color: 'var(--ember)' }}>*</span></label>
                <input
                  className="sab-input"
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.75rem' }} className="form-grid-2">
                <div>
                  ...
                  <input
                    ...
                    onChange={e => setField('client_abn', e.target.value)}
                  />
                </div>
                <div>
                  <label className="sab-label">Client Email <span style={{ color: 'var(--text3)' }}>(optional)</span></label>
                  <input
                    className="sab-input"
              ...
              <div>
                <label className="sab-label">Client Address <span style={{ color: 'var(--text3)' }}>(optional)</span></label>
                <textarea
                  className="sab-input"
                  rows={2}
          ...
          <div style={{ background: '#ffffff', borderRadius: 'var(--r)', border: '1px solid var(--border)', padding: '1.25rem' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 70px 100px 50px 36px', gap: '0.5rem', marginBottom: '0.375rem' }} className="line-items-row">
              {['Description', 'Qty', 'Unit Price (ex GST)', 'GST', ''].map(h => (
                <span key={h} className="sab-label" style={{ margin: 0, fontSize: '0.6875rem' }}>{h}</span>
              ))}
            </div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem', marginBottom: '0.875rem' }}>
              {form.line_items.map((item) => (
                <div key={item.id} style={{ display: 'grid', gridTemplateColumns: '1fr 70px 100px 50px 36px', gap: '0.5rem', alignItems: 'center' }} className="line-items-row">
                  <input
                    className="sab-input"
                    placeholder="e.g. Labour — Deck Construction"
                  ...
                  <label style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer' }}>
                    <input
                      type="checkbox"
                      checked={item.has_gst}
                      onChange={e => updateItem(item.id, { has_gst: e.target.checked })}
                  ...
                  <button
                    ...
                    style={{
                      ...
                      cursor: 'pointer', color: 'var(--text3)', borderRadius: '4px',
                      fontSize: '1.125rem', lineHeight: 1, display: 'flex', alignItems: 'center', justifyContent: 'center',
                      opacity: form.line_items.length === 1 ? 0.3 : 1,
                    }}
                    title="Remove line"
                  >
                    ×
            ...
            <button onClick={addItem} className="btn btn-ghost" style={{ fontSize: '0.8125rem', padding: '0.35rem 0.875rem' }}>
              + Add Line
            </button>
          ...
          <div style={{ background: '#ffffff', borderRadius: 'var(--r)', border: '1px solid var(--border)', padding: '1.25rem' }}>
            ...
            <div>
              <label className="sab-label">Notes <span style={{ fontSize: '0.75rem', fontWeight: 400, color: 'var(--text3)' }}>(optional)</span></label>
              <textarea
                className="sab-input"
                rows={2}
